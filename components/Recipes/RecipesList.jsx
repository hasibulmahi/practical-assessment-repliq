...
const RecipesList = () => {
  ...
  // Query for recipes, runs only when searchQuery is updated
  const { data = [], isLoading, error } = useQuery({
    ...
    queryFn: () => {
      if (searchQuery) {
        ...
        return HttpKit.getTopRecipes(); // Fallback to top recipes if no search input
      }
    },
    enabled: true, // Allows the query to run automatically
  });

  const handleSearch = (e) => {
    e.preventDefault(); // Prevent the default form submission
    setSearchQuery(searchInput); // Set the search query to the current input value
  };
        ...
        <div>
          <form onSubmit={handleSearch} className="w-full mt-12">
            <div className="relative flex p-1 rounded-full bg-white border border-yellow-200 shadow-md md:p-2">
              ...
              />
              <button
                type="submit" // This will trigger the handleSearch function
                title="Search"
